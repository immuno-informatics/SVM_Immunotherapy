...
def mutation_vector_base_per_patient(contig_values_to_experiment_with, dimension_of_embedding_vectors, patients, hotspots, random_contigs, contig_file=False,to_remove=None):
    if contig_file:
        ...
        data = pd.read_csv("data/braun_mutations_alternative_scoring_narrow_broad.tsv", sep="\t", low_memory=False)
        #data = pd.read_csv("data/contig_features_reduced2.tsv", sep="\t", low_memory=False)
    #random_contigs = True
    if random_contigs:
        ratio = len(data.loc[data["contig"].notna()])/len(data)*100
        np.random.seed(800)
        data["contig_1"] = [np.random.randint(0,100) for _ in range(len(data))]
        data["contig"] = data["contig_1"].map(lambda x: "YES" if x<ratio else np.nan)
        new_ratio = len(data.loc[data["contig"].notna()])/len(data)*100
    ...
    #contig_values_to_experiment_with = ["unique_peptides","promiscuity","n_unique_HLA"] 
    #contig_values_to_experiment_with = ["unique_peptides"]
    #contig_values_to_experiment_with = ["promiscuity"]
